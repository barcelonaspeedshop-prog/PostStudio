...
// maxBuffer: 50MB to prevent hangs from ffmpeg stderr output
// timeout: 120s per command to prevent indefinite hangs
        ...
        if (chapterMedia.length === 0) {
          ...
          for (let j = 0; j < chapterMedia.length; j++) {
            ...
            if (media.isVideo) {
              ...
              // Image — static display: scale to fit, pad with dark background
              // Use -stream_loop to repeat the single image frame for the needed duration
              await execAsync(
